...
stmt = 'select objectid, obs_text from ObservationsSire2 where vetid = 6158'
...
for row in results:
  ...
  upd_stmt = f"""update ObservationsSire2 set obs_text='{clean_file(row[1]).replace("'", '')}' where vetid = 6158  and objectid='{row[0]}'"""
  #upd_stmt = f"""update ProblemResolutionSteps set Remarks = '{clean_file(row[1]).replace("'", '')}' where ProblemResolutionStepId = '{row[0]}'"""
  #print(upd_stmt)
  #print('------------')
